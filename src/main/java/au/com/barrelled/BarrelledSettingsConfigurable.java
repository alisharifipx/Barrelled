...
public class BarrelledSettingsConfigurable implements Configurable {
    ...
    public JComponent createComponent() {
        ...
        gbc.gridy++;
        gbc.insets = JBUI.insetsTop(6);
        relativePathRadio = new JRadioButton("Relative path to the selected directory");
        panel.add(relativePathRadio, gbc);

        gbc.gridy++;
        JLabel hint = new JLabel("Specified in the 'Generated Barrel File' dialog");
        panel.add(hint, gbc);

        gbc.gridy++;
        gbc.insets = JBUI.insetsTop(8);
        customPathRadio = new JRadioButton("Specific directory within project:");
        ...
        customPathField.setText(getDisplayCustomPath(settings.getCustomPath()));
        ...
        Runnable updateFields = this::updateFieldAvailability;
    ...
    public boolean isModified() {
        ...
        return currentMode != settings.getOutputMode()
            || !getStoredCustomPathFromField().equals(settings.getCustomPath())
    ...
    public void apply() {
        ...
        settings.setCustomPath(getStoredCustomPathFromField());
    ...
    private String getDisplayCustomPath(String storedPath) {
        String base = project.getBasePath() != null ? project.getBasePath() : "";
        if (storedPath.isEmpty() || base.isEmpty()) {
            return storedPath.isEmpty() ? base : storedPath;
        }

        return base + "/" + storedPath;
    }

    private String getStoredCustomPathFromField() {
        ...
        if (abs.startsWith(base + "/")) {
            return abs.substring(base.length()).replaceAll("^/+", "");
        }
