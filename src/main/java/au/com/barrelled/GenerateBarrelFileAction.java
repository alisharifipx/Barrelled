...
public class GenerateBarrelFileAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent event) {
        ...
        for (VirtualFile child : children) {
            ...
            if (child.isDirectory()
                || name.matches("index\\.(ts|tsx|js|jsx)")
        ...
        VirtualFile initialOutputFolder = resolveOutputFolder(project, settings, folder);
        String initialSourceRelativePrefix = getRelativePath(initialOutputFolder, folder);

        VirtualFile existingBarrel = initialOutputFolder.findChild(barrelFileName);
            ...
            try {
                ...
                if (!existingContent.isBlank()) {
                    ...
                    for (ExportItem item : allExports) {
                        String path = initialSourceRelativePrefix + item.getFileName();
        ...
        BarrelSelectDialog dialog = new BarrelSelectDialog(settings, allExports);
        if (!dialog.showAndGet()) {
            return;
        }

        VirtualFile outputFolder = resolveOutputFolder(project, settings, folder);
        String sourceRelativePrefix = getRelativePath(outputFolder, folder);
        LinkedHashSet<String> managedPaths = new LinkedHashSet<>();
        for (ExportItem item : allExports) {
            managedPaths.add(sourceRelativePrefix + item.getFileName());
        }

        existingBarrel = outputFolder.findChild(barrelFileName);

        List<ExportItem> selected = dialog.getSelectedItems();
        if (selected.isEmpty() && existingBarrel == null) {
            return;
        }
        ...
        WriteCommandAction.runWriteCommandAction(project, "Generate Barrel File", null, () -> {
            try {
                ...
                if (indexFile == null) {
                    ...
                    String mergedContent = BarrelFileMerger.merge(existingContent, finalContent, managedPaths);
    ...
    private static VirtualFile resolveOutputFolder(Project project, BarrelledSettings settings, VirtualFile folder) {
        return switch (settings.getOutputMode()) {
            case RELATIVE -> getVirtualFile(settings, folder);
            case CUSTOM_PATH -> {
                VirtualFile found = LocalFileSystem.getInstance()
                    .findFileByPath(settings.resolveCustomPath(project));
                yield (found != null && found.isDirectory()) ? found : folder;
            }
            default -> folder;
        };
    }
